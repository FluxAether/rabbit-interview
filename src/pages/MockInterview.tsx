...
export default function MockInterview() {
  ...
  if (session.phase === 'setup') return (
    <div className="w-full p-8"><div className="card mx-auto max-w-4xl p-8">
      ...
      <p className="mt-1 text-sm text-[#64748b]">根据职位、简历和 JD 动态出题，支持文本或麦克风回答。</p>
      ...
      <div className="mt-4 grid grid-cols-2 gap-4">
        <label className="flex items-center gap-2 rounded-xl border p-3 text-sm"><input type="checkbox" checked={session.config.voiceInputEnabled} onChange={e => updateConfig({ voiceInputEnabled: e.target.checked })} />麦克风实时转写</label>
        <label className="flex items-center gap-2 rounded-xl border p-3 text-sm"><input type="checkbox" checked={session.config.speechEnabled} onChange={e => updateConfig({ speechEnabled: e.target.checked })} />系统朗读题目</label>
      </div>
      <details className="mt-4 rounded-xl border p-4"><summary className="cursor-pointer text-sm font-medium">面试上下文</summary><textarea className="mt-3 h-28 w-full rounded-xl border p-3 text-sm" placeholder="简历" value={session.config.resumeContext} onChange={e => updateConfig({ resumeContext: e.target.value })} /><textarea className="mt-3 h-28 w-full rounded-xl border p-3 text-sm" placeholder="职位描述" value={session.config.jobDescription} onChange={e => updateConfig({ jobDescription: e.target.value })} /></details>
      {session.error && <div className="mt-4 rounded-xl bg-red-50 p-3 text-sm text-red-700">{session.error}</div>}
  ...
  if (session.phase === 'completed' && session.report) return (
    <div className="w-full p-8"><div className="card mx-auto max-w-5xl p-8">
      <div className="flex items-start justify-between"><div><h1 className="text-2xl font-semibold">面试报告</h1><p className="text-sm text-[#64748b]">{session.config.role} · {session.config.company || '未指定'}</p></div><div className="text-4xl font-semibold text-[#6366f1]">{session.report.overallScore}</div></div>
      <p className="mt-5 rounded-xl bg-[#f8fafc] p-4">{session.report.summary}</p>
      <div className="mt-5 grid grid-cols-2 gap-4"><section className="rounded-xl border p-4"><h2 className="font-medium">优势</h2>{session.report.strengths.map(v => <div key={v} className="mt-2 text-sm">• {v}</div>)}</section><section className="rounded-xl border p-4"><h2 className="font-medium">优先改进</h2>{session.report.priorityImprovements.map(v => <div key={v} className="mt-2 text-sm">• {v}</div>)}</section></div>
      <div className="mt-5 space-y-3">{session.turns.map((turn, index) => <details key={turn.question.id} className="rounded-xl border p-4"><summary className="cursor-pointer font-medium">第 {index + 1} 题 · {turn.feedback?.overallScore ?? 0} 分</summary><p className="mt-3">{turn.question.text}</p><p className="mt-2 rounded-lg bg-[#f8fafc] p-3 text-sm">{turn.answer?.text}</p><p className="mt-2 text-sm text-[#475569]">{turn.feedback?.summary}</p></details>)}</div>
      <div className="mt-6 flex gap-3"><button onClick={exportReport} className="flex-1 rounded-xl border py-2">导出 Markdown</button><button onClick={reset} className="flex-1 rounded-xl bg-[#6366f1] py-2 text-white">{copy.new}</button></div>
    </div></div>
  )
  ...
  return (
    <div className="w-full p-8"><div className="card mx-auto max-w-6xl p-8">
      <div className="flex items-center justify-between"><div><h1 className="text-2xl font-semibold">{copy.title}</h1><div className="text-sm text-[#64748b]">第 {session.turns.length}/{session.config.questionCount} 题 · {Math.floor(session.elapsedSeconds / 60)}:{String(session.elapsedSeconds % 60).padStart(2, '0')}</div></div><button onClick={() => finishInterview(session.turns, false)} className="rounded-xl bg-red-50 px-4 py-2 text-sm text-red-600">{copy.end}</button></div>
      <div className="mt-6 grid grid-cols-[1.2fr_0.8fr] gap-5">
        <section className="rounded-2xl border p-5"><div className="text-xs font-medium uppercase text-[#6366f1]">{session.currentQuestion?.stage}</div><div className="mt-3 text-xl leading-relaxed">{session.currentQuestion?.text || '正在生成问题…'}</div>{session.currentQuestion?.intent && <div className="mt-3 text-sm text-[#64748b]">考察重点：{session.currentQuestion.intent}</div>}<button onClick={() => session.currentQuestion && speakQuestion(session.currentQuestion)} disabled={!session.config.speechEnabled || busy} className="mt-4 rounded-xl border px-3 py-2 text-sm disabled:opacity-40">🔊 重播题目</button></section>
        <section className="rounded-2xl border p-5"><div className="font-medium">上一题反馈</div>{session.turns.slice().reverse().find(turn => turn.feedback)?.feedback ? (() => { const fb = session.turns.slice().reverse().find(turn => turn.feedback)!.feedback!; return <><div className="mt-3 text-3xl font-semibold text-[#6366f1]">{fb.overallScore}</div><p className="mt-2 text-sm">{fb.summary}</p><div className="mt-3 text-xs text-[#64748b]">{fb.improvements.join(' · ')}</div></> })() : <p className="mt-3 text-sm text-[#64748b]">提交回答后获得逐题评分。</p>}</section>
      </div>
      <section className="mt-5 rounded-2xl border p-5"><div className="flex justify-between"><div className="font-medium">{copy.answer}</div><div className="text-xs text-[#64748b]">{session.draftAnswer.length}/4000</div></div><textarea maxLength={4000} value={session.draftAnswer} onChange={e => patch({ draftAnswer: e.target.value })} disabled={busy} className="mt-3 h-40 w-full rounded-xl border p-3 disabled:bg-[#f8fafc]" placeholder={session.config.voiceInputEnabled ? '直接说话，转写结果可编辑…' : '输入你的回答…'} />{session.interimTranscript && <div className="mt-2 text-sm text-[#64748b]">正在识别：{session.interimTranscript}</div>}<button onClick={submitAnswer} disabled={busy || !session.draftAnswer.trim()} className="mt-4 w-full rounded-2xl bg-[#6366f1] py-3 font-medium text-white disabled:opacity-40">{session.phase === 'evaluating' ? '正在分析…' : copy.submit}</button></section>
      {session.error && <div className="mt-4 rounded-xl bg-red-50 p-4 text-sm text-red-700">{session.error}<button onClick={() => patch({ phase: session.currentQuestion ? 'answering' : 'setup', error: null })} className="ml-3 underline">{copy.retry}</button></div>}
    </div></div>
  )
}
