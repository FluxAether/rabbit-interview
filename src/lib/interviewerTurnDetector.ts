...
const NEW_QUESTION_START_PATTERN = /^(?:另一个问题|另外一个问题|下一个问题|再问一个问题|新问题|another question\b|next question\b|new question\b)/i
...
export function shouldInterruptForInterviewerContinuation(
  ...
  if (NEW_QUESTION_START_PATTERN.test(normalized)) return false
