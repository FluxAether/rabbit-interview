...
| 商品代码 | 支付金额 | 发放内容 |
| --- | --- | --- |
| `CREDITS_2900` | ¥89 | 2,900 积分 |
| `CREDITS_11000` | ¥199 | 11,000 积分 |
| `BYOK_LIFETIME` | ¥7 | 当前账号永久解锁自备 API Key，不附赠积分 |

购买的积分在支付验签或服务端主动查询确认成功后立即到账，永久有效，不创建新订阅周期。支付回调、订单幂等键和账号行锁继续防止重复发放；同一账号已有待支付 BYOK 订单时复用该订单，已解锁则拒绝再次购买。

余额只使用 `CREDITS`，一个积分等于 60,000 个整数单位。STT 每接收 1 毫秒音频扣 1 单位；LLM 沿用原有 token 用量核算，每个计费 token 扣 60 单位。因此 1 分钟 STT 或 1,000 个 LLM token 均为 1 积分。请求仍先预留、再结算并释放未使用部分，STT 与 LLM 竞争同一钱包，按原有到期顺序分配。原始音频和 token 用量继续进入审计记录。

注册后完成邮箱验证并设置密码，账号首次激活时赠送 100 积分，有效期 30 天。重发邮件、重复提交、重置密码和再次登录均不重复发放。
...
4. 同步部署 Gateway、landing 和桌面构建，把 `PRICING_POLICY_VERSION` 更新为 `2026-09-credits-v1`，重启 Gateway 后再开放流量。旧未支付订单保留原金额和赠送量快照，后续成功支付发放对应永久积分。
