# OnCue 云端 STT & LLM 商业化订阅产品与技术落地方案
...
### 1.1 背景与核心诉求
OnCue 是一款专注于面试实时辅助与智能对话增强的应用。前期系统主要依赖本地运行或单机配置，为了提升泛化识别准确率、降低用户终端硬件门槛（尤其是中低配 CPU/GPU 设备），并为**国内用户群体提供开箱即用、低延迟且具备商业可持续性的专业服务**，系统需接入国内主流云端实时语音识别（STT/ASR）与大语言模型（LLM）能力，构建完整的用户鉴权、流式代理网关与会员订阅计费闭环。
