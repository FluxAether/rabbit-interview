...
- **内部已核验：**本文中的价格、额度、政策、公式和外部产品边界，已逐项对照五份研究笔记记录的官方 HTML、帮助中心和开发文档；少数地区化或受访问限制的价格仅由官方搜索索引交叉核验，并在相关表格后标明。RabbitInterview 项目事实已对照项目研究笔记及相关仓库文件。
...
2. **RabbitInterview 不需要寻找所谓“一个无限订阅”。**  
   与当前实现最一致的方案，是保留 BYOK 和 Apple 本地路径，并为需要开箱即用体验的用户提供 Rabbit Hosted Gateway。正式 Hosted 应使用与开发环境和个人席位隔离的生产 API 账户，并继续分别计量 `STT_AUDIO_MS` 和 `LLM_TOKEN_UNITS`；当前真实凭证、合同和账单验证尚未完成。这复用现有代码 seam，也避免把上游成本和供应商凭证强制集中到所有用户。
...
4. **先测量，再承诺。**  
   试点期应保留双配额硬闸和无自动续费，收集 P50/P95 单用户 source-duration、单双音源比例、每次生成的 input/output/reasoning tokens、估算 usage 占比、取消浪费、错误重试、支付退款和供应商发票。只有目标 cohort 的 **P95 单用户全量 COGS 及其对应毛利**在约定样本窗口和最低样本量下仍达到内部阈值后，才讨论自动续费、补充包、年约或预留容量。
...
来源：[OpenAI pricing](https://openai.com/chatgpt/pricing/) · [Anthropic pricing](https://www.anthropic.com/pricing) · [Gemini subscriptions](https://gemini.google/subscriptions/) · [Microsoft individual pricing](https://www.microsoft.com/en-us/microsoft-365-copilot/pricing/individuals) · [Microsoft enterprise pricing](https://www.microsoft.com/en-us/microsoft-365-copilot/pricing/enterprise) · [Perplexity pricing](https://www.perplexity.ai/enterprise/pricing) · [Perplexity organization pricing](https://www.perplexity.ai/help-center/en/articles/10352988-creating-a-new-organization) · [Perplexity Enterprise Max](https://www.perplexity.ai/help-center/en/articles/12310544-what-is-enterprise-max.html)

这些价格只能用于相同地区的粗略比较。Google、Microsoft 等页面会随 locale、登录状态和促销返回不同价格；Google 美国部分价格在本次研究中仅由官方搜索索引交叉核验，直接抓取页按访问地区返回了其他币种。正式采购必须在目标国家、目标账户和付款渠道核验结账页或 Order Form。
...
来源：[GitHub plans](https://docs.github.com/en/copilot/get-started/plans) · [GitHub models and pricing](https://docs.github.com/en/copilot/reference/copilot-billing/models-and-pricing) · [Cursor models and pricing](https://cursor.com/docs/models-and-pricing) · [Cursor team pricing](https://cursor.com/docs/account/teams/pricing) · [Windsurf pricing](https://windsurf.com/pricing) · [Windsurf usage](https://docs.windsurf.com/windsurf/accounts/usage) · [Windsurf MSA](https://windsurf.com/MSA) · [Claude plans](https://support.claude.com/en/articles/11049762-choose-a-claude-plan) · [Claude Team](https://support.claude.com/en/articles/9266767-what-is-the-team-plan) · [Claude Code costs](https://docs.anthropic.com/en/docs/claude-code/costs) · [Gemini Code Assist pricing](https://codeassist.google/products/business) · [Gemini Code Assist quotas](https://developers.google.com/gemini-code-assist/resources/quotas)
...
| `PRO_QUARTER` | ¥199 / 90 天 | 3,000 source-min = 50 source-h | 8M units | ¥3.98 / source-h；¥24.88 / 1M units；¥66.33/每 30 天等效 |
...
4. 内部与供应商计量差异阈值获得批准；
5. 适用的上游 DPA/数据处理条款、子处理者、保留、地域和隐私评审已批准；
6. production flags 仍由明确审批开启。
...
**调价门：**在税、汇率、支付、退款、重试、运维和支持成本计入后，目标 cohort 的 **P95 单用户全量 COGS 及其对应毛利**须在预先约定的样本窗口和最低样本量下仍达到内部阈值；否则调整配额或售价，不用平均用户补贴尾部重度用户。
...
- 阶段 B 处理真实 Hosted 内容前，先完成适用的上游 DPA/数据处理条款、子处理者、保留和地域评审；企业客户真实出现后，再增加面向客户的 DPA/附录、SSO/SCIM、审计导出、客户可选驻留和合同 SLA，不把消费级隐私设置包装成企业承诺。
...
- 外部证据对照研究笔记记录的供应商官方 HTML、帮助中心和开发文档；少数地区化或受访问限制的价格仅由官方搜索索引交叉核验，已在相关表格后标明。没有使用第三方价格比较站作为核心证据。
