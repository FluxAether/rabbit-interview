...
const store = fs.readFileSync(path.join(root, 'src/stores/useAppStore.ts'), 'utf8')
...
assert.match(dashboard, /historyStatus === 'ready'/)
assert.match(dashboard, /dashboard\.loadError/)
assert.match(store, /historyStatus: 'loading' \| 'ready' \| 'error'/)
assert.match(store, /loadHistory: \(records\) => set\(\{ history: records, historyStatus: 'ready' \}\)/)
assert.match(app, /setHistoryLoadError\(\)/)
assert.doesNotMatch(dashboard, />24</)
assert.doesNotMatch(dashboard, />82</)
assert.doesNotMatch(dashboard, /Alex Morgan|>AM</)
assert.doesNotMatch(app, /Alex Morgan|>AM</)
assert.doesNotMatch(i18n, /Alex Morgan|Welcome back, Alex|欢迎回来，Alex|歡迎回來，Alex|Premium Plan|高级会员|高級會員/)
