...
export const authCopy: Record<AuthLang, AuthCopy> = {
  en: {
    ...
    subscribe: {
      ...
      hostedBody: "1 credit = 1 minute of speech recognition or 1,000 LLM tokens. Both use the same balance.",
      byokTitle: "Bring your own keys",
      byokBody: "Pay ¥7 once to unlock API keys for this account permanently. Your provider bills usage directly; no credits are used.",
      grantTitle: "100 welcome credits",
      grantBody: "Verify your email and set a password to receive 100 credits once, valid for 30 days. Apple on-device speech requires sign-in but uses no credits and needs no BYOK purchase.",
  ...
  'zh-CN': {
    ...
    subscribe: {
      ...
      checkPayment: "查询支付结果",
      orderNo: "订单号",
      hostedBody: "1 积分 = 1 分钟语音识别，或 1,000 个 LLM token。语音和回答共用同一余额。",
      byokTitle: "自备密钥",
      byokBody: "¥7 一次购买，为当前账号永久解锁 API Key。用量由供应商直接计费，不消耗积分。",
      grantTitle: "新用户赠送 100 积分",
      grantBody: "完成邮箱验证并设置密码后，一次性赠送 100 积分，有效期 30 天。Apple 本地语音识别只需登录，不消耗积分，也无需购买 BYOK。",
  ...
  'zh-TW': {
    ...
    subscribe: {
      ...
      checkPayment: "查詢付款結果",
      orderNo: "訂單編號",
      hostedBody: "1 積分 = 1 分鐘語音辨識，或 1,000 個 LLM token。語音與回答共用同一餘額。",
      byokTitle: "自備金鑰",
      byokBody: "¥7 一次購買，為目前帳號永久解鎖 API Key。用量由供應商直接計費，不消耗積分。",
      grantTitle: "新用戶贈送 100 積分",
      grantBody: "完成電子郵件驗證並設定密碼後，一次贈送 100 積分，有效期 30 天。Apple 本機語音辨識只需登入，不消耗積分，也無需購買 BYOK。",
